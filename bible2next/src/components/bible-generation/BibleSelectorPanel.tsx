            ...
            {finalSelectedBibles.map((bible, index) => (
              <li key={bible.dbVersionId} className="p-2 flex justify-between items-center text-sm">
                ...
                <div className="space-x-1">
                   <button onClick={() => onReorderBibles(bible.dbVersionId, 'up')} disabled={index === 0} className="text-gray-500 hover:text-gray-700 p-1 disabled:opacity-50">▲</button>
                   <button onClick={() => onReorderBibles(bible.dbVersionId, 'down')} disabled={index === finalSelectedBibles.length - 1} className="text-gray-500 hover:text-gray-700 p-1 disabled:opacity-50">▼</button>
                   <button onClick={() => onRemoveBible(bible.dbVersionId)} className="text-red-500 hover:text-red-700 p-1">✕</button>
                </div>
              </li>
            ))}
          </ul>
